 ...
 *
 * note concept of the LZ triple, noted that making a struct that can store a
 * piece of compressed data may help.
 *
 ...
 *
 * lz77_compress takes in a text (uint8_t*) and it's size, and then outputs a
 * string which represents the compressed text. Ptr length width controls how
 * large a match can be.
 *
 * lz77_decompress takes in a compressed lz77 text and decompresses it by
 * looping through progressively each match and replacing them with the
 * signature which represents the pattern
 *
 ****************************************************************************************************/

/******************************************************************************************
 * Main lempel-ziv 77 compression algorithm function, takes in an uncompressed
 * text via a char* and then compresses it by replacing seen sequences with byte
 * data representing instructions to copy that sequence. Compression works as
 * the space required for a match signature is less than the data itself, but
 * only works well on binary data or large text data due to the need for lots of
 * repeating data.
 *
 * inputs:
 * - uint8* (char) uncompressed_text, uint32_t uncompressed_size (error
 * handling), uint8* (char) compressed_text, uint8_t pointer_length_width (size
 * of matches) outputs:
                       ...
                       const uint8_t token_length_width)
{
    ...
    uint16_t token_ptr, token_copy_pos, temp_token_copy_pos, token_length,
        temp_token_length;
    uint32_t data_ptr, output_size, buffer_pos, token_lookahead_ref,
        look_behind, look_ahead;

    /* consts for the max parameters of a token (based on token_length_width
     * param) */
    ...
    *(compressed_text + 4) = token_length_width;
    ...
    /* Looping through the entire uncompressed text until our buffer pos reaches
     * the end */
    for (buffer_pos = 0; buffer_pos < uncompressed_size; ++buffer_pos)
    {
        ...
        for (temp_token_copy_pos = 1;
             (temp_token_copy_pos < TOKEN_COPY_POS_MAX) &&
             (temp_token_copy_pos <= buffer_pos);
             ++temp_token_copy_pos)
        {
            ...
            for (temp_token_length = 0; uncompressed_text[look_ahead++] ==
        ...
        {
            token_ptr = (token_length == 1)
                            ? 0
                            : ((token_copy_pos << token_length_width) |
                               (token_length - 2));
        ...
        {
            token_ptr = (token_copy_pos << token_length_width) |
                        (token_length ? (token_length - 1) : 0);
            token_lookahead_ref = buffer_pos;
        }

        /* once token is built, write it in to the compressed text and move
         * data_ptr forward*/
        *((uint16_t *)(compressed_text + data_ptr)) = token_ptr;
        data_ptr += 2;

        /* add the look ahead byte and then update output size */
        *(compressed_text + data_ptr++) =
            *(uncompressed_text + token_lookahead_ref);
        output_size += 3;
    }
...
/******************************************************************************************
 * Main Lempel-Ziv 77 decompression algorithm, simpler than compression, reads
 * in compressed text, gets the uncompressed size stored in the first 4 bytes,
 * then loops expanding each match signature to the full uncompressed state by
 * reading back to the stored match.
 *
 * inputs:
 * - File ptr *in
 * outputs:
...
{
    /* variable declarations for algorithm */
    uint8_t token_length_width; /*token length width, how much to copy in to a
                                   token*/

    /* token_ptr stores individual tokens, token length / pos specifies how far
       back and how much to copy for a match, mask is used to extract length
       bits */
    ...
    */
    ...
    /* uncompressed size is stored in first 4 bytes of compressed text, grab
     * data */
    ...
    /* fifth byte is the token length - how large matches are in the
     * compression*/
    ...
    /* we keep looping as long as our position is less than the final,
     * uncompressed size */
    buffer_pos = 0;
    while (buffer_pos < uncompressed_size)
    {
        ...
        token_copy_pos =
            token_ptr >> token_length_width; /* where we need to copy from */
        token_length = token_copy_pos ? ((token_ptr & token_length_mask) + 1)
                                      : 0; /* how much to copy */
        ...
        if (token_copy_pos)
        {
            ...
            /* offset is current pos minus token's copy pos; loop until gone
             * through length specified*/
            copy_offset = buffer_pos - token_copy_pos;
            for (; token_length > 0; --token_length)
            {
                /* add to uncompressed text each byte as we go through */
                uncompressed_text[buffer_pos++] =
                    uncompressed_text[copy_offset++];
            }
        }
 ...
 * fsize is a helper function that simply gets the length of a file using
 * ftell/fseek
 *
 * file_lz77_compress takes in a file, outputs a file, and specifies how much
 * memory to use and the pointer length width > is used in a loop to iteratively
 * compress the file based on ptr length
 *
 * file_lz77_decompress takes in a file (compressed), outputs a file which is
 * decompressed
 *
 ****************************************************************************************************/

/******************************************************************************************
 * This helper function takes in a file, and gets the size of the file by
 * seeking the file pointer to the end, getting the length, and then setting the
...
uint32_t lz77_compress_until_optimised(uint8_t *uncompressed_text,
                                       ...
                                       const size_t malloc_size)
{
    ...
    /* iterate over bit sizes until we find the next bitsize increases size
     * instead of decrease - return previous one instead */
    uint32_t i = MIN_BITSIZE;
    for (; i < MAX_BITSIZE; ++i)
    {
        printf("Compressing to bitsize: %d\n", i);
        cur_size = lz77_compress(uncompressed_text, uncompressed_size,
                                 compressed_text, i);
        ...
        /* if old compression better, we know it is the best, return old
         * compression */
        else
        {
...
/******************************************************************************************
 * This function takes in data about a file, a maximum malloc size, and pointer
 * length, and uses this information to write a new compressed file out of an
 * uncompressed one using the lempel-ziv compression algorithm. This method is
 * meant to be used with progressively larger pointer lengths to optimise the
 * compression size.
 *
 * inputs:
 * - char* filename_in, char* filename_out (file IO), size_t malloc_size (max
 * size of new file), uint8_t pointer_length_width used for algorithm (how far
 * to check for match) outputs:
...
uint32_t file_lz77_compress(const char *filename_in, const char *filename_out,
                            const size_t malloc_size)
{
    /* vars for use in method, *in and *out are the files, we have pointers for
     * the texts themselves, and then sizes*/
    FILE *in, *out;
    ...
    /*use this file to verify uncompressed size, malloc for the uncompressed
     * text that is going to be read in */
    ...
    if ((uncompressed_size !=
         fread(uncompressed_text, 1, uncompressed_size, in)))
        return 0;

    /* close file */
    fclose(in);

    /* this is where we actually compress the text, malloc_size is a large
     * buffer, then we pass in data to func */
    compressed_text = malloc(malloc_size);

    compressed_size = lz77_compress_until_optimised(
        uncompressed_text, uncompressed_size, compressed_text, malloc_size);
...
/******************************************************************************************
 * This function takes in a compressed file, and uses it to decompress it in to
 * a new file.
 *
 * inputs:
...
{
    /* vars for use in method, *in and *out are the files themselves, then the
     * texts and sizes*/
    FILE *in, *out;
    ...
    /* compression has stored the compressed size in the first 4 bytes of the
     * compressed text, read these in */
    ...
    if (lz77_decompress(compressed_text, uncompressed_text) !=
        uncompressed_size)
    {
        printf("Decompression failed! Uncompressed size (%d) did not equal "
               "returned size!\n",
               uncompressed_size);
        return 0;
    }
    ...
    if (fwrite(uncompressed_text, 1, uncompressed_size, out) !=
        uncompressed_size)
    {
